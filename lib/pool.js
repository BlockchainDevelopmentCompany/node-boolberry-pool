...
function getFullScratchpad2(callback)
{
    ...
    apiInterfaces.binRpcDaemon('/getfullscratchpad2', {}, function(er, buff)
    {
        ...
        //log('debug', 'Job Refresher', 'Buff type: ' + Object.prototype.toString.call(buff));
        ...
        //log('debug', 'Job Refresher', 'Extracted json: ' + json_str_buff);
...
function processBlockTemplate(template)
{
    ...
    getFullScratchpad2(function (error) {
        ...
        /*//-------------
        log('debug', 'Job Refresher', 'Requesting scratchpad old way....');
        getFullScratchpad(function (error, result) {
            if (error) {
                log('error', 'Job Refresher', 'Failed to get scratchpad: ' + error);
            } else {
                var scratchpad2 = new Buffer(result.scratchpad_hex, 'hex');
                delete result.scratchpad_hex;
                result.hi;
                log('debug', 'Job Refresher', 'Scratchpad fetched Oldway ok');
                log('debug', 'Job Refresher', 'RawFetched: height = ' + scratchpadHeight.height + ', scratchpad.lenght=' + scratchpad.length);
                log('debug', 'Job Refresher', 'OldwayFetched: height = ' + result.hi.height + ', scratchpad.lenght=' + scratchpad2.length);
            }
        });
        */
        //--------------
...
/*
    ...
    getFullScratchpad(function (error, result) {
        if (error) {
            ...
            log('debug', 'Job Refresher', 'Scratchpad fetched ok');
        }
    });
    */
